...
const gananciaA = 10;
const gananciaB = 15;
const horasMoler = 100;
const horasEnvasado = 80;
...
function restriccionManoObra(x, y) {
  return 1 * x + 3 * y <= horasMoler;
}

function restriccionMaquina(x, y) {
  return 2 * x + 1 * y <= horasEnvasado;
}
...
console.log(`La solución óptima es producir ${solucionOptima[0]} unidades de Café Clásico y ${solucionOptima[1]} unidades de Café descafeinado para maximizar las ganancias, con una ganancia total de $${funcionObjetivo(solucionOptima[0], solucionOptima[1])}.`);
